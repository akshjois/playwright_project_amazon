...
export class NavigateMainPage {
    ...
    async searchBoxAction(text:string){
        if (text !== '') {
    ...
    async addProdtoCart(expectedText:string){
    ...
    async addProdtoCartFromChildWindow(expectedText:string){
          await this.waitforproducts(); 
          const count = await this.getProdCount();
